...
#define REG_OP_MODE               0x01  //Controla modo de operação (sleep/standby/TX/RX)
#define REG_FRF_MSB               0x06  //Parte alta da frequência de operação
#define REG_FRF_MID               0x07  //Parte média da frequência de operação
#define REG_FRF_LSB               0x08  //Parte baixa da frequência de operação
#define REG_PA_CONFIG             0x09  //Configuração de potência de saída
#define REG_LNA                   0x0C  //Configuração do amplificador de baixo ruído (LNA)
#define REG_FIFO_ADDR_PTR         0x0D  //Ponteiro para posição atual na FIFO
#define REG_FIFO_TX_BASE_ADDR     0x0E  //Endereço base para transmissão na FIFO
#define REG_FIFO_RX_BASE_ADDR     0x0F  //Endereço base para recepção na FIFO
#define REG_FIFO_RX_CURRENT_ADDR  0x10  //Endereço atual de recepção na FIFO
#define REG_IRQ_FLAGS             0x12  //Flags de interrupção (TX/RX done, CRC error)
#define REG_RX_NB_BYTES           0x13  //Número de bytes recebidos no último pacote
#define REG_PKT_SNR_VALUE         0x19  //Valor SNR do pacote recebido
#define REG_PKT_RSSI_VALUE        0x1A  //Valor RSSI do pacote recebido
#define REG_MODEM_CONFIG_1        0x1D  //Configuração do modem (bandwidth, coding rate)
#define REG_MODEM_CONFIG_2        0x1E  //Configuração do modem (spreading factor, CRC)
#define REG_PREAMBLE_MSB          0x20  //Parte alta do comprimento do preâmbulo
#define REG_PREAMBLE_LSB          0x21  //Parte baixa do comprimento do preâmbulo
#define REG_PAYLOAD_LENGTH        0x22  //Comprimento do payload para modo TX
#define REG_MAX_PAYLOAD_LENGTH    0x23  //Comprimento máximo do payload permitido
#define REG_DIO_MAPPING_1         0x40  //Mapeamento dos pinos DIO para interrupções
#define REG_VERSION               0x42  //Versão do chip (deve ser 0x12)
#define REG_PA_DAC                0x4D  //Controle do DAC de saída de potência

//Modos de Operação
#define MODE_LORA                 0x80  //Habilita modo LoRa (bit 7)
#define MODE_SLEEP                0x00  //Modo de baixo consumo
#define MODE_STDBY                0x01  //Modo standby (ativo, pronto para TX/RX)
...
#define IRQ_RX_DONE_MASK          0x40  //Flag de recepção concluída
#define IRQ_TX_DONE_MASK          0x08  //Flag de transmissão concluída
#define IRQ_PAYLOAD_CRC_ERROR_MASK 0x20  //Flag de erro de CRC no payload
...
// ============================================================================
//Inicializa o módulo LoRa com configurações padrão
bool lora_init() {
    ...
    /* --- Configuração do modem (BW 125 kHz, CR 4/5, CRC on, SF7) --- */
...
//Coloca o módulo em modo de baixo consumo
void lora_sleep() {
    rmf95_write_reg(REG_OP_MODE, MODE_LORA | MODE_SLEEP);
}

//Coloca o módulo em modo standby (ativo mas não TX/RX)
void lora_idle() {
